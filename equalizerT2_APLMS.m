%
% Tb/2-spaced equalizer (APLMS algorithm)
%
clear all, close all
...
xi=zeros(length(y)/2,1); 
N=31;           % Equalizer order
N1=N+1;          % Equalizer length
M=4;
Delta=round((N/2+(length(c)+2*length(pT))/L)/2);
w=zeros(N1,1); nn=round((N1+Delta+M)/2);nn0=nn;
tdl=zeros(N1,M);
for n=N1+Delta+M:2:length(y)-Delta-N1
    tdl=[y(n:-1:n-N1+1) tdl(:,1:end-1)];
    x=(w'*tdl).';
    e=s(nn-Delta:-1:nn-Delta-M+1)-x;
    w=w+2*mu*tdl*inv(tdl'*tdl+0.01*eye(M))*conj(e);
    xi(nn-nn0+1)=abs(e(1)).^2;
    nn=nn+1;
end
figure,semilogy(xi)
