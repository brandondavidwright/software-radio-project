%
% TR_DD: Timing recovery using early-late gating.
%
Tb=0.0001; L=100; Ts=Tb/L; fs=1/Ts; fc=100000; 
Dfc=0; N=8*L; phic=0; sigma_v=0; alpha=0.25; c=1; 
...
for k=start:L:length(tau)*L-L
    ...
    tau(kk+1)=tau(kk)+mu*real((sk-y(k+tauTb))*(y(k+tauTb+dtau)-y(k+tauTb-dtau))');
    kk=kk+1;
end
