%
% TR_GB: Gradient-based timing recovery.
%
...
beta=0.95;               
mu=0.01; Ly=length(y);
kk=1;y0=0;y1=0;start=5*L+1;tau=0.3*ones(1,floor((Ly-start)/L));
for k=start:L:length(tau)*L-5*L
    tauTb=round(tau(kk)*L);
    y0=sqrt(1-beta^2)*y(k+tauTb)-beta*y0;
    y1=sqrt(1-beta^2)*y(k+tauTb+L/2)-beta*y1;
    tau(kk+1)=tau(kk)+mu*real(y0*y1');
    kk=kk+1;
end
